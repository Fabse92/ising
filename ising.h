...
typedef struct
{
    ...
    char sweepPar, sweepMode, hystMode, clusterMode, filmMode;
...
void getParameters(int argc, char **argv, Parameters *para)
{
    ...
    para->sweepPar = 'T'; para->sweepMode = 'n'; para->hystMode = 'n'; para->clusterMode = 'n'; para->filmMode = 'n';   
    ...
    if(argc < 3 
      ...
      || (argc > 11 && sscanf(argv[11], "%c", &para->clusterMode) != 1) || (para->clusterMode != 'y' && para->clusterMode != 'n')
      || (argc > 12 && sscanf(argv[12], "%c", &para->filmMode) != 1) || (para->filmMode != 'y' && para->filmMode != 'n'))
        usage(argv[0]);    
        
    printf("\nattempting to execute %s %d %d %d %c %f %f %f %f %c %c %c %c \n\n", argv[0], para->N, para->dim, para->steps, para->sweepPar, para->sweep_init, para->sweep_end, para->sweep_step, para->C, para->sweepMode, para->hystMode, para->clusterMode, para->filmMode); 
}
